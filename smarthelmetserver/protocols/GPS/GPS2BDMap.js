  ...
  var  x_pi = 3.14159265358979324 * 3000.0 / 180.0;
    ...
    function gcj_decrypt_exact(gcjLat, gcjLon) {
        ...
        while (1) { 
            wgsLat = (mLat + pLat) / 2;             
            wgsLon = (mLon + pLon) / 2; 
            vartmp = gcj_encrypt(wgsLat, wgsLon);
            dLat = tmp.lat - gcjLat;             
            dLon = tmp.lon - gcjLon; 
    ...
    function mercator_encrypt(wgsLat, wgsLon) {         
                        ...
                        var y = Math.log(Math.tan((90. + wgsLat) * PI/360.0)) / (PI / 180.); 
                        y = y * 20037508.34 / 180.;         
                        return {'lat' : y, 'lon' : x};         
    } 
 ...
 module.exports = {
    ...
    gPS2BDLoc: function(lat,lon){

        var wLoc = gcj_encrypt(lat,lon);
        var bdLoc = bd_encrypt(wLoc.lat,wLoc.lon);
        return {'bdalt':bdLoc.lat,'bdult':bdLoc.lon};
